...
2a. **If the tier is `heavy` and the current model is not already Opus** [Ph34-hotfix], dispatch to an Opus subagent rather than executing locally on a lighter model: use the `Agent` tool with `subagent_type: implement-step` and `model: opus` set explicitly — this overrides that agent's default `sonnet` frontmatter (per CLAUDE.md's Model Routing: the `model` parameter always takes precedence over an agent file's own default). This is the per-step dispatch path CLAUDE.md's Advisor Guidance and `skills/plan.md`'s Tier Assignment Guide both assume exists for heavy-tier and security-sensitive steps reached from an otherwise lighter-tier session. Same self-contained-prompt requirement as above.
